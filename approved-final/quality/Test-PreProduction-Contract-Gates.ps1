...
$result = [ordered]@{
  ...
  strictInteractions = if ($mobileGate) { [int]$mobileGate.strictInteractions } else { 0 }
  highIntentControls = if ($mobileGate) { [int]$mobileGate.highIntentControls } else { 0 }
  curatedIntentOutcomes = if ($mobileGate) { [int]$mobileGate.curatedIntentOutcomes } else { 0 }
