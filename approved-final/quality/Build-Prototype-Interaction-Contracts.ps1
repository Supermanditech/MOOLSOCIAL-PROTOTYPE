...
$intentOverridePath = Join-Path $Root 'quality\intent-completion-overrides.json'
...
$intentOverrides = Get-Content -Raw -LiteralPath $intentOverridePath | ConvertFrom-Json
foreach ($entry in @($intentOverrides.entries)) {
  if ([string]::IsNullOrWhiteSpace([string]$entry.runtimeType)) { continue }
  $screenKey = [string][int]$entry.screen
  if (-not $contracts.Contains($screenKey)) { $contracts[$screenKey] = [ordered]@{} }
  $label = [string]$entry.label
  if (-not $contracts[$screenKey].Contains($label)) {
    $contracts[$screenKey][$label] = [ordered]@{
      type = [string]$entry.runtimeType
      priority = 'P0'
      ticket = "IC-$screenKey-$($contracts[$screenKey].Count + 1)"
      note = [string]$entry.note
    }
  }
  $contract = $contracts[$screenKey][$label]
  $contract.type = [string]$entry.runtimeType
  $contract.intentOutcome = $true
  foreach ($propertyName in @('note','route','primaryLabel')) {
    $value = [string]$entry.$propertyName
    if (-not [string]::IsNullOrWhiteSpace($value)) { $contract[$propertyName] = $value }
  }
}
