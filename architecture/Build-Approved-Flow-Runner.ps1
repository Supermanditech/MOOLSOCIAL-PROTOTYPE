...
$allNodes = foreach ($screen in 0..106) {
  $base = $screenContracts[$screen]
  [pscustomobject][ordered]@{
    index = $screen
    screen = $screen
    ...
    nextScreen = if ($screen -lt 106) { $screen + 1 } else { $null }
    productionModule = $base.productionModule
    routeId = $base.routeId
    routePath = $base.routePath
    routeState = $base.routeState
...
$allFlow = [pscustomobject][ordered]@{
  id = 'all-approved'
  title = 'All Approved Screens 00-106'
...
Write-Output "Built $(@($flows).Count) operational flows plus all-approved coverage for 107 screens."
